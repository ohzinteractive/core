export default class FrustumPointFitter
{
  ...
  fit_points(points, camera_quaternion, vertical_fov, aspect)
  {
    ...
    let v_fov = THREE.Math.degToRad(vertical_fov/2);
    ...
    for (let i = 0; i < points.length; i++) 
    {
      ...
      let line = new THREE.Line3(points[i].clone(), points[i].clone().add(inverse_dir));
  ...
  get_vertical_correction(points, down_plane, up_plane, camera_quaternion)
  {
    ...
    let up_dir = new THREE.Vector3(0,1,0).applyQuaternion(camera_quaternion);
    for (let i = 0; i < points.length; i++) {

      let down_line   = new THREE.Line3(points[i].clone(), points[i].clone().add(up_dir.clone().multiplyScalar(-5000000)));
      let up_line     = new THREE.Line3(points[i].clone(), points[i].clone().add(up_dir.clone().multiplyScalar(5000000)));
  ...
  get_hozirontal_correction(points, left_plane, right_plane, camera_quaternion)
  {
    ...
    let right_dir = new THREE.Vector3(1,0,0).applyQuaternion(camera_quaternion);

    for (let i = 0; i < points.length; i++) {

      let left_line  = new THREE.Line3(points[i].clone(), points[i].clone().add(right_dir.clone().multiplyScalar(-5000000)));
      let right_line = new THREE.Line3(points[i].clone(), points[i].clone().add(right_dir.clone().multiplyScalar(5000000)));
